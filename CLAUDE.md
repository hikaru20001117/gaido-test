...
**メインエージェント（SubAgentでない場合）は、ユーザーから最初のメッセージを受け取ったら、メッセージの内容に関わらず、まず `/orchestration-guide`、`/phase-workflow`、`/gaido-menavi-character` を実行すること。ユーザーのメッセージ内容から判断して手順を省略してはならない。SubAgentはこれらのSkillを実行してはならない。**

| フェーズ | 実行方式 | 動作 |
|---------|----------|------|
| 壁打ちフェーズ | Skill `/analyze` | 専門家による質問・回答ループ |
| 要件確認フェーズ | Subagent `phase-backlog` | Epic/PBI/Task定義 → 品質検証 → Issue一括登録 → 親子関係構築 |
| 実装フェーズ | Subagent `phase-develop` | PBI単位でSubagent呼出 → 各SubagentがTask実装・コミット・PR作成 → レビュー（設定に応じてユーザー/AI(`from-ecc-code-reviewer` + `from-ecc-security-reviewer`)/なし） |
...
│   │   ├── from-ecc-code-reviewer.md    # コードレビューSubagent
│   │   ├── from-ecc-security-reviewer.md # セキュリティレビューSubagent
│   │   ├── from-ecc-architect.md # アーキテクチャレビューSubagent
...
│       ├── gaido-dev-utils-issue-workshop/  # 対話型PBI/Spike作成（独立ツール）
│       ├── orchestration-guide/  # オーケストレーション手順（メインエージェント専用）
│       ├── phase-workflow/      # フェーズ遷移ワークフロー（メインエージェント専用）
│       ├── gaido-menavi-character/           # キャラクター設定（メインエージェント専用）
...
│       ├── gaido-proposal-read-excel/  # Excel方眼紙設計書読み取り（gaido-proposal-initから自動呼び出し）
...
│       ├── gaido-estimate-pl-generator/   # 見積・PLシート自動生成（独立ツール）
...
│       ├── gaido-proposal-project-plan/  # 提案書からプロジェクト計画書を自動生成（gaido-proposal-init完了後に連続実行）
│       └── gaido-proposal-why-ctc/  # SharePointからWhy CTC素材収集（gaido-proposal-init Step 3.5から呼び出し）
│       └── gaido-persona-generator/  # ペルソナ生成（エゴグラム＋壁打ちで人格シミュレーション作成）
│       └── （チケット系は gaido-report-ticket plugin 経由で提供）
...
このコンテナで参照可能な env var (`LAUNCH_MODE` / `CLAUDE_CODE_TOKEN` / `GITHUB_TOKEN` / `TARGET_REPO_URL` / `CONTEXT7_API_KEY` / `PENCIL_LICENSE_TOKEN` 等) は、gaido アプリ側の Single Source of Truth `desktop/src/shared/config/bindings.ts` から自動導出されコンテナに渡される (issue #760 / セットアップウィザードアーキテクチャ)。

Box の refresh_token は env var で渡さない (Issue #831)。代わりに `~/.gaido/box/shared/credentials.json` がコンテナの `/workspace/target_repo/.box/credentials.json` に bind mount され、`tools/box_client.py` が直接読み書きする。詳細は `.claude/rules/box-integration.md` を参照。

- `LAUNCH_MODE` は起動モード判定の SSoT (`dev` = アプリ開発モード / `quick` = 資料作成モード)。entrypoint.sh と welcome-message / orchestration-guide / sync-target-repo の各 SKILL.md は `[ "$LAUNCH_MODE" = "dev" ]` で分岐する (#805)。
- 新規 env var を **コンテナ側で利用したい**場合は、gaido リポジトリの `bindings.ts` にエントリを追加してもらう必要がある (entrypoint.sh だけ書き換えても渡らない)。
- `set -u` は使用しない方針 (モード別 env が未定義渡し前提)。env var の defined / empty 判定は `[ -n "$VAR" ]` を使う。
- 詳細は gaido リポジトリの `CLAUDE.md`「env var を追加する手順」を参照。
...
| `from-ecc-code-reviewer` | コードレビュー（AIレビュー設定時、PBI単位でPR差分の品質・保守性チェック・GitHub Review投稿） |
| `from-ecc-security-reviewer` | セキュリティレビュー（AIレビュー設定時、PBI単位でPR差分のOWASP Top 10・脆弱性チェック・GitHub Review投稿） |
| `from-ecc-architect` | アーキテクチャレビュー（システム設計、スケーラビリティ、技術的意思決定の評価・提案） |
| `phase-test-design` | テスト設計（テストケースIssue作成） |
| `phase-test-run` | テスト実装（E2Eテスト実装・実行） |
| `phase-finalize` | 成果まとめ（README.md作成・セッションエクスポート） |
| `existing-source-analysis-db` | ソースからDB構造解析→db.md生成（Step 4でSubAgentとしてspawn） |
| `existing-source-analysis-screens` | ソースから画面構成解析→screens.md生成（Step 4でSubAgentとしてspawn） |
| `existing-source-analysis-architecture` | ソースからアーキテクチャ解析→architecture.md生成（Step 4でSubAgentとしてspawn） |
| `existing-source-analysis-openapi` | ソースからWebAPI定義解析→openapi.yaml生成（Step 4でSubAgentとしてspawn） |
...
| `/gaido-dev-utils-issue-workshop` | 対話でPBI+Spikeを練り上げるワークショップ（メインワークフロー外の独立ツール） |
...
| `/gaido-menavi-character` | キャラクター設定・メェナビの口調ルール（メインエージェント専用、開発開始時に実行） |
...
| `/gaido-legal-review` | 契約書・NDA等の法務文書レビュー（gaido-legal plugin で導入、メインワークフロー外の独立ツール） |
| `/gaido-legal-oss-check` | OSSライセンス互換性チェック（gaido-legal plugin で導入、メインワークフロー外の独立ツール） |
| `/gaido-legal-policy` | 利用規約・プライバシーポリシーのドラフト生成（gaido-legal plugin で導入、メインワークフロー外の独立ツール） |
| `gaido-legal-agent` agent (Task ツール経由) | 法務エージェント（契約書レビュー・OSSライセンスチェック・ポリシー生成を内部で集約。gaido-legal plugin で導入） |
| `/gaido-report-ticket:gaido-report-ticket-of-bug-from-user` | 利用者との対話でGAiDoアプリのバグ情報を聞き取り、gaidoリポジトリにIssue登録（gaido-report-ticket plugin） |
| `/gaido-report-ticket:gaido-report-ticket-of-bug-from-problem` | problems.mdの各項目をgaidoリポジトリのIssueとして登録（finalizeフェーズ後に実行。gaido-report-ticket plugin） |
| `/gaido-report-ticket:gaido-report-ticket-of-feature-request-from-user` | 利用者との対話でGAiDoアプリへの機能要望を聞き取り、gaidoリポジトリにIssue登録（gaido-report-ticket plugin） |
| `/gaido-persona-generator` | エゴグラム＋壁打ちでペルソナファイルを生成（壁打ち相手・顧客シミュレーション用） |
| `/gaido-estimate-pl-generator` | 複数社の原価見積を統合し、PL表・顧客提示用見積Excelを生成（独立ツール） |
...
| `existing-source-analysis-screens-operations` | 画面構成解析手順（同上） |
| `existing-source-analysis-architecture-operations` | アーキテクチャ解析手順（同上） |
| `existing-source-analysis-openapi-operations` | WebAPI定義解析手順（同上） |
...
| `rules/ms365-integration.md` | MS365連携（tools/ms365_client.pyの使い方・CLIコマンド・エラー対応） |
| `rules/error-handling.md` | エラー発生時のリカバリ案内（認証エラー等） |
