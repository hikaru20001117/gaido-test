---
name: gaido-proposal-rfp-qa-generator
...
**既存Skillとの関係:**
- `/gaido-proposal-init`: RFP読み込み後にオプション呼び出し。案件名・入力資料を引き継ぐ
- `/gaido-proposal-read-excel`: 入力資料がExcel設計書形式の場合に呼び出し
- `/project-advisor`: Go判定後に `/gaido-proposal-init` → 本スキルの流れが典型的な利用パターン
...
- 顧客回答を受け取り、提案書に反映したい
- `/gaido-proposal-init` から呼び出された（Step 3.5 経由）

## フロー

```mermaid
flowchart TD
    A["/gaido-proposal-rfp-qa-generator 開始"] --> A1{"gaido_progress.jsonから<br>案件名を取得できるか?"}
    ...
    F -->|"NO"| H
    G --> H["全入力資料を読み込み・解析<br>.xlsxあれば /gaido-proposal-read-excel を呼び出し"]
...
エラー時の対処は `/gaido-proposal-init` のStep 3と同様に対応する。
...
`.xlsx` が1件以上存在する場合、Skillツールで `/gaido-proposal-read-excel` を自動実行する（ユーザー確認不要）。
`/gaido-proposal-read-excel` が `ai_generated/input/design_summary.md` を生成したら、以降の資料読み込みで追加コンテキストとして使用する。
...
```bash
# テンプレートをDL
python3 tools/box_client.py download {ファイルID} --output ai_generated/proposals/{案件名}/qa/template.xlsx

# テンプレートのカラム構造を読み取り、QA JSONをExcelに出力
python3 .claude/skills/gaido-proposal-rfp-qa-generator/scripts/qa_excel_handler.py write-excel \
...
**Box連携がない場合:**
AskUserQuestionで以下を表示する:

```
質疑一覧の準備が完了しました。

Box未連携のためローカルに保存しました。ファイルは以下に出力されています:
  保存先: ai_generated/proposals/{案件名}/qa/qa_list.md または qa_list.xlsx

Box連携を有効にすると、この成果物が自動でBoxに保存されます（GAiDoアプリの Step 4 で設定できます）。
...
```bash
python3 tools/box_client.py download {ファイルID} \
  --output ai_generated/proposals/{案件名}/qa/qa_answered.{xlsx|md}
```

#### 回答内容の解析

```bash
python3 .claude/skills/gaido-proposal-rfp-qa-generator/scripts/qa_excel_handler.py import-answers \
...
`box_client.py upload` はディレクトリ一括アップロードに対応していないため、成果物ファイルを個別にアップロードする:

```bash
# 成果物ファイルのみ個別にアップロード（qa_draft.json 等の中間ファイルは上げない）
for f in qa_list.xlsx qa_list.md reflection_summary.md answers.md; do
  [ -f "ai_generated/proposals/{案件名}/qa/$f" ] && \
    python3 tools/box_client.py upload \
      "ai_generated/proposals/{案件名}/qa/$f" \
      --folder-path "GAiDo/{案件名}/qa"
done
```

完了後、`Box path:` 行を引用して保存先を案内し、ユーザーに `qa/reflection_summary.md` の内容を提示して終了する。
...
保存先:
  - `ai_generated/proposals/{案件名}/qa/reflection_summary.md`（提案への反映サマリ）
  - `ai_generated/proposals/{案件名}/qa/answers.md`（回答取込結果）

完了後、ユーザーに上記のローカル保存ファイル内容を提示して終了する。
