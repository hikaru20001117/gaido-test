...
**判定完了:**
  ...
  → Go系/Proceed系の場合:
    ...
    `/gaido-proposal-init` を Skill ツールで実行（以降の進捗は `/gaido-proposal-init` が管理）
...
flowchart TD
    ...
    N2 -->|"スキップ"| O
    N3 --> O
    O -->|"Go系/Proceed系"| P["/gaido-proposal-init を自動実行"]
    O -->|"見送り系"| Q["完了"]
    D --> R["完了"]
    E --> R
    K --> R
...
- **「この内容でPPTを作成する」**: `/gaido-proposal-init` と同じパイプラインで生成する
  ...
  3. Skillツールで **`/gaido-proposal-slide-generator`** を実行し、上記HTMLデザインをもとにPPTXを生成する
...
13. **Phase 5実施済みの場合**: AskUserQuestionでPPT生成を確認する
    - 「プロジェクトプロファイルPPTを確定スコアで更新する」: story.mdを確定スコアで更新し、`/frontend-design` → `/gaido-proposal-slide-generator` のパイプラインでPPTX再生成
    - 「スキップする」: そのまま次へ
14. 「Git操作＆Box連携（共通）」に従いcommit & push & Box upload
15. 判定結果がGo系/Proceed系の場合、Skillツールで `/gaido-proposal-init` を実行する。確定レポートのファイルパスが `/gaido-proposal-init` の入力資料として自動検出される
