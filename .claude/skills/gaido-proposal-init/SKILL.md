...
本スキル内の全ての `python3 tools/box_client.py upload`（story.md / デザインPNG / draw.io素材 / PPTX 等）は、`.box/credentials.json` が存在する場合のみ実行する。詳細は [.claude/rules/box-integration.md](.claude/rules/box-integration.md) の「## 成果物の自動アップロード方針」を参照。

- 未連携/失効時はアップロードをスキップし、ローカル保存に留める。各箇所で「Boxの…フォルダに保存しました」と事実と異なる成功報告をせず、ローカル保存パスを案内すること。
- 処理の最後（Step 12）で一度だけ「Box未連携のためローカルに保存しました。Box連携を有効にすると、この成果物が自動でBoxに保存されます（GAiDoアプリの Step 4 で設定できます）」と伝える。
- アップロードが失敗してもスキル全体を止めず、ローカルパスを案内して処理を続行する。
...
Box連携がある場合は「BoxのGAiDo/{案件名}/proposal フォルダにスライド骨格（story.md）を保存しました」とユーザーに伝える。Box未連携の場合は「Box未連携のためローカル（`ai_generated/proposals/{案件名}/story.md`）に保存しました」と伝える（成功報告のハルシネーション防止）。
...
2. PPTX のBoxアップロードは `/gaido-proposal-slide-generator` が手順1の生成時に自動で行う（`.box/credentials.json` がある場合のみ）。**ここで重複してアップロードしてはならない。** アップロード成功時は `Box path:` 行を引用して保存先を案内する（`.claude/rules/box-integration.md` の「## アップロード後のパス案内（ハルシネーション防止）」参照）。Box未連携時はローカルパス `ai_generated/proposals/{案件名}/output/proposal.pptx` を案内する。

3. ユーザーに確認を依頼する
   「提案書（proposal.pptx）が完成しました。ご確認ください」

4. 修正が必要な場合は `.claude/skills/gaido-proposal-init/references/QualityChecklist.md` で再確認し、該当スライドを修正して `/gaido-proposal-slide-generator` で再生成する（再生成時に slide-generator が再アップロードする）。その後に再確認する
