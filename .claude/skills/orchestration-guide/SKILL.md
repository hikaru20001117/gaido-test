...
0c. /record-costs "既存ソース解析フェーズ"
    ...
    # ASK は APIドキュメント生成の ON/OFF のみ。
    AskUserQuestionで確認:
      header: "Step A: APIドキュメント生成"
      question: "APIドキュメントを生成しますか？\nソースコードからTypeDoc/GoDoc等でAPIドキュメントを生成し、既存構造の解析精度を高めます。\nファイル数に応じてツール実行に数分かかる場合があります。\n生成しなくても（構造解析のみでも）既存システムの改修は可能です。"
      options:
        - "実行する": "APIドキュメントを生成する（解析精度が上がる・推奨）"
        - "スキップする": "APIドキュメント生成をスキップし、構造解析のみ行う（高速）"
    → 「実行する」の場合:
      `/existing-source-analysis-operations` スキルを「APIドキュメント生成あり」で実行する（メインAgentがオーケストレーターとして動作し、SubAgentをspawnして処理を委任する）
    → 「スキップする」の場合:
      `/existing-source-analysis-operations` スキルを「省略モード」で実行する（コメント付与・APIドキュメント生成ともスキップ、file_structure.md 生成のみ）
...
- **`/existing-source-analysis-operations`**（Skill、メインAgent直接実行）: メインAgentがオーケストレーターとして直接実行し、SubAgentをspawnして処理を委任する。ステップ 0c の ASK は APIドキュメント生成の ON/OFF（実行する=APIドキュメント生成あり / スキップする=省略モード）
