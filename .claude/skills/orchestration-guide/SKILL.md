...
0-pre. セマンティック検索コンテナ起動（バックグラウンド・アプリ開発モード時のみ）:
    Bashで以下を実行し、現在の起動モードを確認する（entrypoint.sh が設定した `$LAUNCH_MODE` env var を参照）:
    ```bash
    echo "LAUNCH_MODE=$LAUNCH_MODE"
    ```
    `$LAUNCH_MODE=dev` の場合のみ以下を実行する:
    ...
    `$LAUNCH_MODE=quick`（またはそれ以外）の場合: このステップをスキップする（アプリ開発を行わないためセマンティック検索は不要）
   ...
   → 選択結果に応じてflow_typeを確定:
     ...
     - 「営業案件を相談する」→ flow_type = "project_advisor"
       ...
       → 終了（/project-advisorのGo判定で /gaido-proposal-init に自動遷移する場合あり）

     - 「提案書を作成する」→ flow_type = "proposal"
       ※ 確定したflow_typeでui_phasesを上書きするため、意図的に2回目のcompleted記録を行う
       /record-progress "開発内容選択フェーズ" "completed" --flow-type proposal
       /gaido-proposal-init を実行（進捗記録・コスト記録はSkill自身が管理）
       → 終了
...
0c. /record-costs "既存ソース解析フェーズ"
    ...
    AskUserQuestionで確認:
      ...
      question: この質問を表示する前に、以下のBashコマンドを実行して所要時間と完了予定時刻を取得し、結果をquestionに埋め込むこと。
        ```bash
        FILE_COUNT=$(find output_system*/ -type f \( -name '*.ts' -o -name '*.tsx' -o -name '*.go' -o -name '*.py' -o -name '*.js' -o -name '*.jsx' \) -not -path '*/node_modules/*' -not -path '*/.next/*' -not -path '*/dist/*' | wc -l)
        # T6実機実測（2026-04-17〜20、vercel/swr 254ファイル）:
        #   1ファイル約57.9秒、248ファイルで5hウインドウ100%消費（1ファイル≈0.40%）
        # ユーザー提示用の目安: 1ファイル=1分、5hウインドウ消費率=0.4%/ファイル
        MINUTES=$FILE_COUNT
        # 5hウインドウのレートリミット消費率（小数第1位まで表示）
        RATE_LIMIT_PERCENT=$(awk "BEGIN{printf \"%.1f\", $FILE_COUNT * 0.4}")
        # コンテナTZがUTCのため明示的にJSTを指定
        FINISH_TIME=$(TZ='Asia/Tokyo' date -d "+${MINUTES} minutes" '+%H:%M JST')
        echo "ソースファイル数: ${FILE_COUNT}ファイル、所要時間の目安: 約${MINUTES}分（${FINISH_TIME}頃に完了見込み）、5hウインドウのレートリミット消費率: 約${RATE_LIMIT_PERCENT}%"
        ```
        上記の出力結果を使って、以下の形式でquestionを構成する。
        "ソースコードにコメント付与とAPIドキュメント生成を行いますか？\n生成することで既存の構造の解析精度が向上します。\nソースファイル数: {FILE_COUNT}ファイル、所要時間の目安: 約{MINUTES}分（{FINISH_TIME}頃に完了見込み）、5hウインドウのレートリミット消費率: 約{RATE_LIMIT_PERCENT}%\n生成しなくても既存システムの改修自体は可能です。"
      options:
        - "実行する": "ソースコードにコメントを付与し、APIドキュメントを生成する（推奨）"
        - "スキップする": "コメント付与・APIドキュメント生成をスキップし、構造解析のみ行う"
    → 「実行する」の場合:
      `/existing-source-analysis-operations` スキルを実行する（メインAgentがオーケストレーターとして動作し、SubAgentをspawnして処理を委任する）
    → 「スキップする」の場合:
      `/existing-source-analysis-operations` スキルのStep 2（コメント付与）をスキップし、Step 1 → Step 3 → Step 4 の順で実行する
...
9a. Task(gaido-legal-agent, "タスク種別: oss-check, 対象: Backlog（PBI/Task Issue）に記載された技術・ライブラリのライセンス互換性を確認してください。要件ファイル ai_generated/requirements/ も参照してください")
...
15. Epic番号順にループ:
    ...
    b. PBI番号順にループ（ユーザーへの表示はIssueタイトルのPBI番号を使用。例: 「PBI 1.2（#2084）」）:
       ...
       - 【AIレビューの場合】AIレビューループ（最大3回）:
         1. Task(from-ecc-code-reviewer, "PR #X をコードレビューしてください。対象PR: #X")
         2. Task(from-ecc-security-reviewer, "PR #X のセキュリティレビューを行ってください。対象PR: #X")
...
16. 全Epic完了後:
    a. Task(gaido-legal-agent, "タスク種別: oss-check, 対象: プロジェクトの実際の依存関係ファイルをスキャンし、ライセンス互換性を確認してください")
...
24a. Task(gaido-legal-agent, "タスク種別: oss-check, 対象: プロジェクトの実際の依存関係ファイル（package.json等）をスキャンし、ライセンス互換性を最終確認してください")
...
- **from-ecc-code-reviewer**: 「PR #{番号} をコードレビューしてください。対象PR: #{番号}」（AIレビューの場合のみ、PBI単位でループ呼出）
- **from-ecc-security-reviewer**: 「PR #{番号} のセキュリティレビューを行ってください。対象PR: #{番号}」（AIレビューの場合のみ、PBI単位でループ呼出）
...
- **`/existing-source-analysis-operations`**（Skill、メインAgent直接実行）: メインAgentがオーケストレーターとして直接実行し、SubAgentをspawnして処理を委任する。ASKでStep Aスキップが選択された場合はStep 2をスキップ
...
- **gaido-legal-agent（Backlog後）**: 「タスク種別: oss-check, 対象: Backlog（PBI/Task Issue）に記載された技術・ライブラリのライセンス互換性を確認してください」
- **gaido-legal-agent（実装完了後）**: 「タスク種別: oss-check, 対象: プロジェクトの実際の依存関係ファイルをスキャンし、ライセンス互換性を確認してください」
- **gaido-legal-agent（テスト後）**: 「タスク種別: oss-check, 対象: プロジェクトの実際の依存関係ファイルをスキャンし、ライセンス互換性を最終確認してください」
- **/project-advisor**（Skill）: 引数なし。営業案件アドバイザーフロー全体を実行。進捗・コスト記録はSkill自身が管理
- **/gaido-proposal-init**（Skill）: 引数なし。提案書作成フロー全体を実行。進捗・コスト記録はSkill自身が管理
...
- 承認の場合: 次フェーズへ遷移
- AIレビューの場合: develop PBI間はAIレビュー（from-ecc-code-reviewer + from-ecc-security-reviewer）→ 自動修正ループ → 通過後に親がマージ → 全PBI完了後のみ動作確認ゲート
...
| from-ecc-code-reviewer / from-ecc-security-reviewer | PRコメントにレビュー結果が投稿されていること |
...
| gaido-legal-agent（各段階） | サマリのJSON内 `risk_level` を確認（green/yellow/red） |
