---
name: existing-source-analysis-operations-step2
description: 既存ソース解析Step 2。コメント付与ルール定義。comment-writer.shのsystem-prompt-fileとして使用される。
user-invocable: false
---

# ソースファイルコメント付与ルール

指定されたソースファイルを読み込み、コメント・ドキュメントを追加してください。

## アクセス制約

指定されたファイルのみアクセスおよびコメント付与すること。下記以外のファイルは読み書き禁止。
...
- **ファイルの読み書きは必ずReadツール→Editツールで行うこと。**
- すべてのコメント文は**日本語**で記述する。ただしソースコード中の識別子（変数名・関数名・クラス名・型名・定数名等）およびコード例は元の構造のまま残し、日本語に翻訳しない
- **ブロックコメントが存在する言語（TypeScript, Go, Python等）では、追加するコメントはすべてブロックコメント形式とする。** 行コメント（`//`単体等）は禁止（GoDocの`//`スタイルは例外。詳細はコメント書式ルールを参照）
- ブロックコメントの**先頭行**に `AUTO_GENERATED:` プレフィックスを付ける。1ブロックにつき1回のみ。`@param`や引数説明の行には付けない
...
プロンプトで指定されたファイルパスをReadツールで読み込む。
