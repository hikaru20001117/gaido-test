...
| └ コードレビュー | Subagent `from-ecc-code-reviewer` | AIレビュー設定時、PBI単位でPR差分の品質・保守性をチェック → GitHub Review投稿 |
| └ セキュリティレビュー | Subagent `from-ecc-security-reviewer` | AIレビュー設定時、PBI単位でPR差分のOWASP Top 10・脆弱性をチェック → GitHub Review投稿 |
...
| 既存ソース解析フェーズ | Skill `/existing-source-analysis-operations`（メインAgent直接実行、SubAgent方式） | ソースコードのみを解析し中間ファイルに出力 |
...
| 案件判定完了 | Skill `/project-advisor`（自律管理） | 最終判定・Go時は /gaido-proposal-init へ遷移 |
| 提案準備フェーズ | Skill `/gaido-proposal-init`（自律管理） | レポート検出 → 案件名確認 → ワークスペース作成 |
| 資料読み込みフェーズ | Skill `/gaido-proposal-init`（自律管理） | 入力資料（RFP等）の読み込み |
| 構成壁打ちフェーズ | Skill `/gaido-proposal-init`（自律管理） | スライド構成の壁打ち → story.md作成 |
| デザインフェーズ | Skill `/gaido-proposal-init`（自律管理） | HTMLモック生成（/frontend-design） |
| 図表・スライド生成フェーズ | Skill `/gaido-proposal-init`（自律管理） | 図表作成 → PPTX生成（/gaido-proposal-slide-generator） |
| 提案書確認フェーズ | Skill `/gaido-proposal-init`（自律管理） | ユーザーによるPPTX確認 |
| 提案書完了 | Skill `/gaido-proposal-init`（自律管理） | Git操作＆Box連携 → 完了 |
...
| 13 | `/existing-source-analysis-operations` 実行前 | 既存ソース解析フェーズ | `/record-costs` | `/record-progress ... "starting"` |
...
| 24-30 | /gaido-proposal-init 各フェーズ境界 | 提案準備〜提案書完了 | Skill自身が管理 | Skill自身が管理 |
